...
env_to_use = 'InvertedDoublePendulumBulletEnv-v0'
...
exploration_mu = 0.2	# mu parameter for the exploration noise process: dXt = theta*(mu-Xt)*dt + sigma*dWt
exploration_theta = 0.2 # theta parameter for the exploration noise process: dXt = theta*(mu-Xt)*dt + sigma*dWt
...
while 1:
  ...
  while 1:
    time.sleep(1. / 60.)
    action_for_state, = sess.run(actions, feed_dict = {state_ph: obs[None],is_training_ph:False})
    a = action_for_state
    print("STEP: "+str(a))
    obs, r, done, _ = env.step(action_for_state)
    if done == True:
      break
    score += r
    frame += 1
